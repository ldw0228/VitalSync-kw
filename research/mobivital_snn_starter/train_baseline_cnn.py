...
class RespirationCNN(nn.Module):
    """Small 1D CNN baseline for waveform reconstruction."""
...
def save_prediction_plot(
    ...
    plt.subplot(2, 1, 1)
    plt.plot(t, signal, label="UWB magnitude input")
    ...
    plt.legend(loc="upper right")
    plt.xlabel("time (s)")
    plt.title("Baseline CNN respiration reconstruction")
    plt.tight_layout()
...
def main() -> None:
    ...
    out_dir = Path(args.out_dir) if args.out_dir else Path(args.csv[0]).resolve().parent / "baseline_cnn_out"
    ...
    best_val = float("inf")
    best_path = out_dir / "baseline_cnn.pt"
