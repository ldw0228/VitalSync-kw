    ...
    for o in SY:
        ...
        for nm,a0,a1 in BLK:
            ...
            while t+WIN<=a1:
                ...
                t+=HOP
    np.savez_compressed(V+'ds2.npz',X=X,ok=ok,y=D0['y'],subj=D0['subj'],blk=D0['blk'])
...
X=D['X'].astype(np.float32); Y=D['y_rate'].astype(np.float32)
SUBJ=D['subj']; BLKn=D['blk']
DS=1; N,Tn,F=X.shape
...
json.dump(dict(enc=which,mae=float(e.mean()),p1=float((e<=1).mean()),
          ...
          open(V+'e2e2_%s.json'%which,'w'),ensure_ascii=False)
